...
- **Prompt injection.** User inserts text that overrides the system prompt. Mitigation: treat user input as untrusted throughout the loop; sanitize before tool calls; isolate tool outputs from the main prompt.
